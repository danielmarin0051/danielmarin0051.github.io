...
function onWindowResize() {
  ...
  camera.aspect = _width / _height;
  camera.updateProjectionMatrix();
  options.perlin.perlins = _pixelRatio !== 1 ? 4.0 : 2.0;
  // console.log("- resize -");
}
...
function animation() {
  ...
  if (z_speed > 0 && camera.position.z >= 15) z_speed *= -1;
  else if (z_speed < 0 && camera.position.z <= 5) z_speed *= -1;
  mat.uniforms["time"].value = 15 + options.perlin.speed * (Date.now() - start);
